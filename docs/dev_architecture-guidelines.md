...
The Intranet Portal utilizes a **Hybrid Modular Architecture** bridging a dominant Modular Monolith with completely decoupled Standalone Microservices. When a new module is proposed, the development team must evaluate its nature, scope, and database coupling to decide where it belongs.

- **The Core Monolith (The "Central Sun"):** Modules that actively mutate foundational System/HR data or require immediate transactional SQL consistencies (e.g., hard `INNER JOINS`) reside in the single main repository. They share the unified Next.js frontend shell and .NET backend API, strictly segregating into domain folders under `(core)` and `(assets)`. *(For a complete breakdown, refer to [docs/dev_architecture-modular-monolith.md](dev_architecture-modular-monolith.md)).*
