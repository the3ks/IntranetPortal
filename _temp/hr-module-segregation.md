...
- `UnifyPersonnelToHR`: consolidated personnel references to HR_Employees and HR_Departments.
- `UpdatePersonnelUnification`: enforced strict 1-to-1 required mapping and removed legacy `UserAccountId` from `HR_Employees`.
...
7. **Personnel Unification & SSOT Stabilization.**
- Legacy bidirectional links have been removed; `Core_UserAccounts.EmployeeId` is now the strict, required **Master Link**.
- All cross-module dependencies (Announcements, Assets, Drink Orders) are now rewired to HR personnel entities via the UserAccount navigation property.
- Legacy `UserAccountId` column has been dropped from `HR_Employees` to ensure a single direction of dependency.
- Manual self-healing Admin Seeder implemented to ensure valid personnel links for administrative accounts.
...
- Added manual `/api/auth/seed-test-admin` endpoint for local development recovery.
...
1. **Phase 4 Functional Depth**: Implementing Leave Request submission/approval UI and Onboarding task management.
