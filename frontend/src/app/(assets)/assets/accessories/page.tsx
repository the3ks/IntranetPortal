      ...
      {showForm && (
        <div className="bg-fuchsia-50/30 p-6 rounded-2xl border border-fuchsia-100/50">
          <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              <label className="text-xs font-bold text-foreground/60 uppercase">Accessory Name *</label>
              <input required className="w-full mt-1 border border-border/50 rounded-lg p-2.5 outline-none focus:border-fuchsia-400 focus:ring-2 focus:ring-fuchsia-100" value={newAcc.Name} onChange={e => setNewAcc({...newAcc, Name: e.target.value})} placeholder="e.g. Logitech MX Master 3" />
            </div>
            <div>
