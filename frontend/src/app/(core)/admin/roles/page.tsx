    ...
    <MainLayout>
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <p className="text-foreground/60 mt-2 text-lg">Administrate Enterprise Security Roles and Data Capability Definitions.</p>
          </div>
        </header>
        ...
        <div className="space-y-8">
          ...
          <div className="w-full">
            <div className="bg-card rounded-3xl shadow-sm border border-border/50 p-8">
              <h2 className="text-xl font-bold text-foreground mb-6">Create System Role</h2>
              <form action={createRole} className="flex flex-col md:flex-row gap-6 items-start">
                <div className="flex-1 w-full space-y-4">
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-foreground/90">Role Identifier</label>
                    <input type="text" name="name" required className="w-full px-4 py-3 rounded-xl border border-border/50 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all font-bold text-foreground" placeholder="e.g. Asset Manager" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-foreground/90">Security Description</label>
                    <input type="text" name="description" className="w-full px-4 py-3 rounded-xl border border-border/50 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-foreground/90" placeholder="Controls warehouse assets..." />
                  </div>
                </div>
            ...
            <div className="bg-card rounded-3xl shadow-sm border border-border/50 overflow-hidden">
              {roles.length === 0 ? (
                <div className="p-16 text-center">
                  <h3 className="text-xl font-bold text-foreground mb-2">No roles registered</h3>
                </div>
              ) : (
                    ...
                    {roles.map((r: any) => (
                      <tr key={r.id} className="hover:bg-indigo-50/40 transition-colors">
                        ...
                        <td className="px-6 py-5 text-sm text-foreground/60 font-medium">
                          {r.description || <span className="text-border italic">No description</span>}
                        </td>
