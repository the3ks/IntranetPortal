    ...
    <MainLayout>
      <div className="max-w-7xl mx-auto py-8 lg:py-12 space-y-10">
        <header className="text-center sm:text-left">
          <h1 className="text-4xl sm:text-5xl font-black text-foreground tracking-tight">Corporate Announcements</h1>
          <p className="mt-4 text-foreground/60 text-lg sm:text-xl font-medium max-w-2xl">The official source for urgent updates, policy shifts, and company-wide news broadcasts from leadership.</p>
        </header>

        {canPostAnnouncement && (
          <div className="bg-card p-6 sm:p-8 rounded-[2rem] shadow-sm border border-border/50 flex flex-col gap-6 relative overflow-hidden">
            ...
            <div className="flex items-center space-x-4 relative z-10">
              ...
              <h2 className="text-xl font-bold text-foreground">Broadcast Update</h2>
            </div>

            <form action={createAnnouncementAction} className="flex flex-col gap-4 relative z-10">
              <input type="text" name="title" required placeholder="Headline (e.g., Q3 Bonus Structure Updated)" className="px-5 py-4 text-lg font-bold rounded-xl border border-border/50 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-foreground shadow-sm bg-background/50 align-top transition-all" />
              <select name="siteId" className="px-5 py-3 rounded-xl border border-border/50 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-foreground/90 shadow-sm bg-background/50 font-medium transition-all appearance-none cursor-pointer">
              ...
              <textarea name="content" required rows={4} placeholder="Write your announcement details here... Markdown is supported internally." className="px-5 py-4 rounded-xl border border-border/50 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-foreground/90 shadow-sm bg-background/50 resize-y transition-all" />
        ...
        <div className="space-y-6">
          {announcements.length === 0 ? (
            <div className="text-center py-20 bg-card rounded-3xl border border-dashed border-border/50">
              <svg className="w-16 h-16 mx-auto text-border mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg>
              <h3 className="text-xl font-bold text-foreground">No signals found</h3>
              <p className="text-foreground/60 mt-2">The company broadcast channel is currently quiet.</p>
            </div>
          ) : (
            announcements.map((ann: any) => (
              <article key={ann.id} className="bg-card p-8 sm:p-10 rounded-[2.5rem] shadow-sm border border-border/50 hover:shadow-md transition-shadow group relative">
                <div className="flex items-start justify-between gap-4 mb-6">
                  <div>
                    <h2 className="text-2xl sm:text-3xl font-extrabold text-foreground leading-tight mb-2 tracking-tight">{ann.title}</h2>
                    <div className="flex flex-wrap items-center gap-3 text-sm font-medium text-foreground/60">
                      ...
                      <time dateTime={ann.createdAt} className="flex items-center">
                        <svg className="w-4 h-4 mr-1.5 text-foreground/40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                  ...
                  {canDeleteAnnouncement && (
                    <form action={deleteAnnouncementAction.bind(null, ann.id)}>
                      <button type="submit" className="text-border hover:text-rose-500 p-2 rounded-xl hover:bg-rose-50 transition-colors shrink-0" title="Delete Broadcast">
